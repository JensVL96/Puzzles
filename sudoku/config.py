

#   Screen info
SCREEN_HEIGHT = 700
SCREEN_WIDTH = int(SCREEN_HEIGHT * 1.6)
SCREEN_RES = (SCREEN_WIDTH, SCREEN_HEIGHT)

#   Colours
BLACK = (0,0,0)
GREY = (50,50,50)
GREEN = (0,255,0)
RED = (255, 0, 0)
WHITE = (255,255,255)
BEIGE = (242,234,224)
ACCENT = (100, 180, 100)

# Box info
BORDER_SIZE = 405
BLOCK_SIZE = BORDER_SIZE / 9
CELL_SIZE = BLOCK_SIZE / 9
THICK_LINE = 7
BOX_INDENT = THICK_LINE / 2
THIN_LINE = 1
NUM_LINES = 10
# Corner coordinates
TOP_LX = (SCREEN_WIDTH / 2) - (BORDER_SIZE / 2)
TOP_LY = (SCREEN_HEIGHT / 2) - (BORDER_SIZE / 2)
TOP_RX = (SCREEN_WIDTH / 2) + (BORDER_SIZE / 2)
TOP_RY = (SCREEN_HEIGHT / 2) - (BORDER_SIZE / 2)
BOT_LX = (SCREEN_WIDTH / 2) - (BORDER_SIZE / 2)
BOT_LY = (SCREEN_HEIGHT / 2) + (BORDER_SIZE / 2)
BOT_RX = (SCREEN_WIDTH / 2) + (BORDER_SIZE / 2)
BOT_RY = (SCREEN_HEIGHT / 2) + (BORDER_SIZE / 2)

# Precomputed line positions (accounts for thick/thin line offsets)
_INDENT_SEQ = [0, 1, 1, 2, 3, 3, 4, 5, 5, 6]
LINE_X = [TOP_LX + i * BLOCK_SIZE + _INDENT_SEQ[i] * BOX_INDENT for i in range(10)]
LINE_Y = [TOP_LY + i * BLOCK_SIZE + _INDENT_SEQ[i] * BOX_INDENT for i in range(10)]

# Numbers
CANDIDATE_SIZE = 20
NUMBER_SIZE = 40
BOLD = 50

# Blinking
BLINK_SPEED = 0.3
BLINK_COLOR = GREEN
ERROR_COLOR = RED

INITIAL_CORDS = (-1, -1)
INITIAL_LOCK = 0