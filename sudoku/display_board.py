...
class Display_board():
            ...
            for j in range(9):
                if grid[i][j] != 0:
                    if type(grid[i][j]) != int:
                        self.draw_candidates(sorted(grid[i][j]), (LINE_X[i], LINE_Y[j]))
                    else:
                        text1 = self.font_num.render(str(grid[i][j]), 1, BLACK)
                        cx = (LINE_X[i] + LINE_X[i + 1]) / 2
                        cy = (LINE_Y[j] + LINE_Y[j + 1]) / 2
                        self.screen.blit(text1, text1.get_rect(center=(cx, cy)))

        for i in range(NUM_LINES):
            thick = THICK_LINE if i % 3 == 0 else THIN_LINE
            pg.draw.line(self.screen, BLACK, (LINE_X[0], LINE_Y[i]), (LINE_X[9], LINE_Y[i]), thick)
            pg.draw.line(self.screen, BLACK, (LINE_X[i], LINE_Y[0]), (LINE_X[i], LINE_Y[9]), thick)
    ...
    def draw_candidates(self, grid, cell):
        ...
        for idx, number in enumerate(grid):
            ...
            self.screen.blit(text1, (cell[0] + col * step + 7, cell[1] + row * step + 4))
    ...
    def find_cell(self, x, y):
        if x == -1 and y == -1:
            return None
        return (LINE_X[x], LINE_Y[y], LINE_X[x + 1] - LINE_X[x], LINE_Y[y + 1] - LINE_Y[y])
