...
top  = [3,1,5,0,0,7,0,2,3,4,0]
side = [0,6,2,1,0,2,1,3,3,5,2]
...
# Supported clues:
# - "F": occupied ship cell (unknown part)
# - "O": single-cell ship
# - "^", ">", "v", "<": ship end facing N/E/S/W
# board[6][8] = "<"  # y,x
# board[5][4] = "<"  # y,x
...
def set_forced_part(r, c, part):
    # Mark a coordinate as a forced ship-part clue.
    # Supported clues:
    # - "F": occupied ship cell (unknown part)
    # - "O": single-cell ship
    # - "^", ">", "v", "<": ship end facing N/E/S/W
    if part not in FORCED_CLUE_MARKERS:
        raise ValueError(f"Unsupported forced part clue: {part}")
    board[r][c] = part
...
def check_partial(occupied_rows, row_hits, col_hits, blocked_rows, ships_remaining):
    ...
    # Forced clue cells must remain placeable until they are occupied.
    for r, c in FORCED_CELLS:
        bit = 1 << c
        if (occupied_rows[r] & bit) == 0 and (blocked_rows[r] & bit) != 0:
            return False
...
def extract_forced_cells():
    ...
    for r in range(board_size):
        for c in range(board_size):
            if board[r][c] in FORCED_CLUE_MARKERS:
                forced.append((r, c))
    return forced

def extract_forced_part_clues():
    clues = []
    for r in range(board_size):
        for c in range(board_size):
            clue = board[r][c]
            if clue in FORCED_CLUE_MARKERS and clue != "F":
                clues.append((r, c, clue))
    return clues

def is_occupied(occupied_rows, r, c):
    if not (0 <= r < board_size and 0 <= c < board_size):
        return False
    return (occupied_rows[r] & (1 << c)) != 0

def forced_clue_matches(occupied_rows, r, c, clue):
    if not is_occupied(occupied_rows, r, c):
        return False

    n = is_occupied(occupied_rows, r - 1, c)
    e = is_occupied(occupied_rows, r, c + 1)
    s = is_occupied(occupied_rows, r + 1, c)
    w = is_occupied(occupied_rows, r, c - 1)
    neighbors = n + e + s + w
    ...
    # Directional end clues: symbol indicates where the ship end points.
    # Example: '^' means this is the north-facing end, so the only neighbor is south.
    if clue == "^":
        return neighbors == 1 and s
    if clue == ">":
        return neighbors == 1 and w
    if clue == "v":
        return neighbors == 1 and n
    if clue == "<":
        return neighbors == 1 and e

    return False

def check_forced_part_clues(occupied_rows):
    for r, c, clue in FORCED_PART_CLUES:
        if not forced_clue_matches(occupied_rows, r, c, clue):
            return False
    return True
...
def generate_largest_ship_candidates(occupied_rows, blocked_rows, row_hits, col_hits, ships, placements_by_length):
    ...
    for placement in placements_by_length[largest_length]:
        ...
        if check_partial(occupied_rows, row_hits, col_hits, blocked_rows, remaining_ships):
...
def solve_all(occupied_rows, blocked_rows, row_hits, col_hits, ships, placements_by_length):
    ...
    if not ships:
        ...
        if not check_forced_part_clues(occupied_rows):
            return
    ...
    for placement in placements_by_length[length]:
        ...
        if check_partial(occupied_rows, row_hits, col_hits, blocked_rows, new_ships):
...
FORCED_PART_CLUES = extract_forced_part_clues()
...
for r, c in FORCED_CELLS:
    # Forced cells are requirements, not pre-placed ships.
    # They must be covered by some placement during the search.
    pass
